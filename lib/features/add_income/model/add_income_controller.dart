...
class AddIncomeController {
  ...
  Future<({int storedMinor, String storedCurrency, double? rateUsed, DateTime? rateTimestamp})>
      _resolveStored(AddIncomeInput input) async {
  ...
  Future<List<Income>> findPotentialDuplicates(
    AddIncomeInput input, {
    int? excludeId,
  }) async {
    final pool = await ref.read(appDatabaseProvider).getAllIncome();
    final key = fingerprintOf(
      occurredAt: input.occurredAt,
      originalAmountMinor: input.originalAmountMinor,
      originalCurrencyCode: input.originalCurrencyCode,
    );
    return findIncomeMatches(pool: pool, fingerprint: key, excludeId: excludeId);
  }
  ...
  Future<void> update(
    ...
    final db = ref.read(appDatabaseProvider);
    final existing = await db.getIncomeById(id);
    if (existing == null) {
      throw StateError('income_not_found');
    }
    final resolved = await _resolveStored(input);
    final original = input.originalCurrencyCode.toUpperCase();

    final oldKey = fingerprintOfIncome(existing);
  ...
  Future<void> delete(int id) {
    return ref.read(appDatabaseProvider).deleteIncomeById(id);
  }
  ...
  /// Mirrors `BulkExpenseController.markNotDuplicate` for income rows.
...
final addIncomeControllerProvider = Provider<AddIncomeController>((ref) {
  return AddIncomeController(ref);
});
