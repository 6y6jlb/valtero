...
- Dashboard: direction tab → filter summary bar above the chart → full-screen sheet; one donut / column / **columns-by-date** / **line** chart (type icons top-right via [ChartOverlayControls](lib/features/expenses_list/ui/chart_overlay_controls.dart); choice persisted) for expenses/income, or cash-flow **donut** (income vs expense, default) / **grouped bars** / **line** (persisted as `cashFlowChartType`); shared [BreakdownChartView](lib/features/expenses_list/ui/breakdown_chart_view.dart) / [DonutBreakdownChart](lib/features/expenses_list/ui/donut_breakdown_chart.dart) / [CashFlowChartView](lib/features/expenses_list/ui/cash_flow_chart_view.dart): hover/touch uses in-plot tooltips; amounts on segments; legend chips (tag/payment icons or flags when available) show amounts under the label and toggle visibility; for expenses/income, tap segment → list with filter; cash-flow donut has no segment drill-down; breakdown/period icons sit in one horizontally scrollable row **above the legend** (never above the plot) via [ChartBreakdownRow](lib/features/expenses_list/ui/chart_breakdown_row.dart) (cash flow: temporal only; period persisted as `cashFlowChartDatePeriod`); horizontal swipe / scroll on the plot cycles breakdown, outside the plot cycles Cash flow / Expenses / Income — both use a finger-driven parallel slide via [InteractiveSlidePager](lib/features/expenses_list/ui/directional_slide_switcher.dart) (vertical scroll offset is kept); when breakdown is category, a compact Subcategories switch leads the legend (off by default = parent categories only); recent operations (leading icon: **subcategory tag → tag → currency flag → payment**; country stays in the subtitle) + “Show expenses”; sticky bottom actions are individual solid FABs via [GlassSurface](lib/widgets/glass_surface.dart) (`primaryContainer` / `onPrimaryContainer`, same as selected tabs — Show / Create / bulk)
...
- **Changelog + bump**: every commit bumps **patch** and updates [`CHANGELOG.md`](CHANGELOG.md); user-visible work also updates in-app What's new ([`release_notes.dart`](lib/features/about_support/model/release_notes.dart)). Minor/major only when the user asks — see [docs/agent-rules/changelog.md](docs/agent-rules/changelog.md). Build-only (`+n`) bumps are optional and do not replace patch-on-commit.
...
7. **If editing an agent rule**: update `docs/agent-rules/<topic>.md` first (source of truth). If a local `.cursor/rules/<topic>.mdc` mirror exists, update its body in the same pass. Never edit only the `.mdc`. If the one-line summary in the topic table below would be wrong, update that row in the same pass.
8. **After finishing a plan / feature**: run full `flutter test` (not only new files), fix every failure including unrelated fixtures broken by schema changes, then re-run until green — see [testing.md](docs/agent-rules/testing.md). Tests cover business logic, processing, and infrastructure (money, filters, rates, schema, sync). Do not add widget or chart-chrome tests. If Flutter/Dart tooling is blocked by the environment, report that and ask the user — **never** copy the Flutter/Dart SDK (or other toolchains) into the repo to work around it — see [tooling-environment.md](docs/agent-rules/tooling-environment.md)
9. **On every commit**: bump **patch** in `VERSION` (unless the user explicitly asked for minor/major), sync `pubspec.yaml`, update [`CHANGELOG.md`](CHANGELOG.md), and for user-visible changes update in-app What's new — see [changelog.md](docs/agent-rules/changelog.md)
...
| [docs/agent-rules/testing.md](docs/agent-rules/testing.md) | Business logic, processing, infrastructure; no widget/chart chrome; **full `flutter test` after a plan** |
| [docs/agent-rules/changelog.md](docs/agent-rules/changelog.md) | Keep a Changelog + short in-app What's new; **patch on every commit**; minor/major only when the user asks |
